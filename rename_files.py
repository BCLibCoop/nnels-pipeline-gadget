...
def get_file_names(patterns, folder):
	...
	if cfg.HOST_OS == 'Darwin':
		...
		#args_temp = ['find', './testdata/files', '-regextype posix-extended', '-regex', '.*/*.zip']
		#args_temp = ' '.join(args)
		#output = subprocess.Popen("find ./testdata/files -regextype posix-extended -regex '.*/*.zip'", shell=True, stdout=subprocess.PIPE).communicate()[0]
		#output = subprocess.Popen(args_temp, shell=True, stdout=subprocess.PIPE).communicate()[0]
		#print output
		#exit()
...
# Return: N/A                                        #
#----------------------------------------------------#
def rename_files(cfg, records, patterns, SCN_index=None, folder='.', sweep=True):
	if cfg.DEBUG_MODE == 'stacktrace':
		print '===================================================='
		print 'Call Summary for rename_files (rename_files.py) '
		print '----------------------------------------------------'
